...
class RNNModel(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim=32, hidden_dim=64, num_layers=4, dropout=0.1):
...
def _validate_and_checkpoint(model, val_loaders, device, loss_fn, config, epoch, avg_train_loss, best_val_loss, patience_counter):
    ...
    current_val_loss = val_metrics.get(val_loss_key, float('inf')) if val_loss_key else float('inf')    # Early stopping logic
    improved = False
...
def train_experiment(config):
    ...
    run_name = f"RNN_{config['lang']}_train{config['train_len']}"
    ...
    with wandb.init(
        ...
        config=config,
        name=run_name,
        group=f"RNN-{config['lang']}-train{config['train_len']}"
    ) as run:
        config = run.config # Use wandb.config for sweeps
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"\n--- Starting RNN Experiment: {run_name} on {device} ---")
...
def main():
    ...
    # RNN configuration as specified
    base_config = {
    ...
    print("\n\n===== STARTING RNN BASELINE EXPERIMENTS =====")
