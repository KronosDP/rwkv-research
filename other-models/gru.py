...
def train_experiment(config):
    ...
    run_name = f"GRU_{config['lang']}_train{config['train_len']}"
    ...
    with wandb.init(
        ...
        config=config,
        name=run_name,
        group=f"GRU-{config['lang']}-train{config['train_len']}"
    ) as run:
        config = run.config # Use wandb.config for sweeps
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"\n--- Starting GRU Experiment: {run_name} on {device} ---")
...
def main():
    """Main function to run GRU experiments."""
    ...
    print("\n\n===== STARTING GRU BASELINE EXPERIMENTS =====")
